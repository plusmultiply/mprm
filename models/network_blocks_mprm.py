...
def spatial_attention(layer_ind, inputs, features, radius, fdim, config, training):
    fdim = int(fdim/4)
    with tf.variable_scope('simple1'):
        features = simple_block(layer_ind, inputs, features, radius, fdim, config, training)
    with tf.variable_scope('unary1'):
        x1 = unary_block(layer_ind, inputs, features, radius, int(fdim/8), config, training)

    with tf.variable_scope('unary2'):
        x2 = unary_block(layer_ind, inputs, features, radius, int(fdim/8), config, training)

    with tf.variable_scope('unary3'):
        x3 = unary_block(layer_ind, inputs, features, radius, fdim, config, training)

    start_ind = tf.constant(0)
    batch_inds_0 = tf.constant(0)
    # start_ind=0
    # batch_inds_0 = 0
    stacked_length = inputs['stacked_length']
    # softmax_axis = tf.constant([0,1])
    # num_batch = stacked_length.shape[0]
    # x = tf.Variable(np.empty((1, features.shape[1]), dtype=np.float32), collections=[])
    x = tf.zeros((0, features.shape[1]), dtype=np.float32)
    xn = tf.zeros((0, features.shape[1]), dtype=np.float32)

    # Define body of the while loop
    def body(start_ind, batch_inds_0, x, xn):
        batch_length = tf.gather(stacked_length, batch_inds_0)
        end_ind = start_ind + batch_length
        # end_ind = start_ind + stacked_length[batch_inds_0]
        query = x1[start_ind:end_ind]
        key = x2[start_ind:end_ind]
        value = x3[start_ind:end_ind]
        key = tf.transpose(key)
        att = tf.matmul(query, key)
        # att = (tf.exp(att) / tf.reduce_sum(tf.exp(att), softmax_axis)) + 1E-5
        att = tf.nn.softmax(att)
        att = tf.matmul(att, value)
        att_n = att / tf.cast(batch_length, dtype=tf.float32)
        x = tf.concat([x, att], 0)
        xn = tf.concat([xn, att_n], 0)
        start_ind = end_ind
        batch_inds_0 += 1

        return start_ind, batch_inds_0, x, xn

    def cond(start_ind, batch_inds_0, x, xn):
        return batch_inds_0 < tf.shape(stacked_length)[0]

    _, _, x, xn = tf.while_loop(cond,
                            body,
                            loop_vars=[start_ind, batch_inds_0, x, xn],
                            shape_invariants=[start_ind.get_shape(), batch_inds_0.get_shape(),
                                              tf.TensorShape([None, features.shape[1]]),
                                              tf.TensorShape([None, features.shape[1]])])

    # assert x.shape() == shortcut.shape()
    with tf.variable_scope('shortcut'):
        shortcut = features
        #assert x.shape[0] == shortcut.shape[0]
        alpha = tf.Variable(tf.constant(0, dtype=tf.float32), name='weights')
        merged = alpha * x + shortcut

    with tf.variable_scope('simple2'):
        merged = simple_block(layer_ind, inputs, merged, radius, fdim, config, training)

    return merged, xn
    # return x


def channel_attention(layer_ind, inputs, features, radius, fdim, config, training):
    fdim = int(fdim/4)
    with tf.variable_scope('simple1'):
        features = simple_block(layer_ind, inputs, features, radius, fdim, config, training)
    start_ind = tf.constant(0)
    ...
    # Define body of the while loop
    def body(start_ind, batch_inds_0, x):
        end_ind = start_ind + tf.gather(stacked_length, batch_inds_0)
        query = features[start_ind:end_ind]
        key = features[start_ind:end_ind]
        value = features[start_ind:end_ind]
        query = tf.transpose(query)
        att = tf.matmul(query, key)
        att_new = tf.broadcast_to(tf.reduce_max(att, -1, keepdims=True), att.get_shape()) - att
        #att_new = att
        # att = tf.exp(att) / tf.reduce_sum(tf.exp(att), softmax_axis)
        att_new = tf.nn.softmax(att_new)
        att_new = tf.matmul(value, att_new)
        x = tf.concat([x, att_new], 0)
        start_ind = end_ind
        batch_inds_0 += 1
        # x = value

        return start_ind, batch_inds_0, x

    def cond(start_ind, batch_inds_0, x):
        return batch_inds_0 < tf.shape(stacked_length)[0]

    _, _, x = tf.while_loop(cond,
                            body,
                            loop_vars=[start_ind, batch_inds_0, x],
                            shape_invariants=[start_ind.get_shape(), batch_inds_0.get_shape(),
                                              tf.TensorShape([None, features.shape[1]])])
    ...
    with tf.variable_scope('shortcut'):
        shortcut = features
        #assert x.shape[0] == shortcut.shape[0]
        beta = tf.Variable(tf.constant(0, dtype=tf.float32), name='weights')
        merged = beta * x + shortcut
...
def dropout_max_per_channel(layer_ind, inputs, features, radius, fdim, config, training):
    #max_features = tf.reduce_max(features, axis=1, keepdims=True)
    c_features_ = tf.concat([features, tf.zeros((1, features.shape[1]), features.dtype)], axis=0)

    # Collect each batch features
    batch_features = tf.gather(c_features_, inputs['out_batches'], axis=0)

    # Average features in each batch
    batch_features_max = tf.reduce_max(batch_features, axis=1)

    activations = features / tf.gather(batch_features_max, inputs['last_batch_ind'])

    channel_mask = tf.cast(tf.greater(config.dropout_threshold, activations), dtype=tf.float32)
    #channel_mask = tf.reduce_min(channel_mask, axis=1, keepdims=True)
    #local_mask = tf.cast(tf.greater(0.5, channel_mask), dtype=tf.float32)
    #neighbor_mask = ind_min_pool(channel_mask, inputs['neighbors'][layer_ind])
...
def point_dropout_max_channel(layer_ind, inputs, features, radius, fdim, config, training):
    #max_features = tf.reduce_max(features, axis=1, keepdims=True)
    c_features_ = tf.concat([features, tf.zeros((1, features.shape[1]), features.dtype)], axis=0)
    ...
    channel_mask = tf.reduce_min(channel_mask, axis=1, keepdims=True)
    #local_mask = tf.cast(tf.greater(0.5, channel_mask), dtype=tf.float32)
    #neighbor_mask = ind_max_pool(local_mask, inputs['neighbors'][layer_ind])

    #features = features * (1-neighbor_mask)

    return channel_mask


def dual_attention_block(layer_ind, inputs, features, radius, fdim, config, training, dropout_prob):
    # Dropout
    ...
    with tf.variable_scope('sa'):
        sa, sa_x = spatial_attention(layer_ind, inputs, features, radius, fdim, config, training)
    with tf.variable_scope('ca'):
        ca = channel_attention(layer_ind, inputs, features, radius, fdim, config, training)
    with tf.variable_scope('psa'):
        psa = tf.concat([features, sa_x], axis=1)
        psa = simple_block(layer_ind, inputs, psa, radius, fdim, config, training)

    #with tf.variable_scope('dropout'):
    #    dp_mask = point_dropout_max_channel(layer_ind, inputs, features, radius, fdim, config, training)

    #def true_(): return tf.ones_like(dp_mask)

    #def false_(): return dp_mask

    # features = tf.cond(np.random.uniform() > dropout_prob, true_fn=true_, false_fn=false_)
    #dp_mask = tf.cond(np.random.uniform() < dropout_prob, true_fn=true_, false_fn=false_)


    #dual = sa+ca
    fdim = config.num_classes
    #with tf.variable_scope('dual_unary'):
        #dual = dual * dp_mask
        #dual_unary = unary_block(layer_ind, inputs, dual, radius, fdim, config, training)
    with tf.variable_scope('sa_unary'):

        #sa = sa * dp_mask
        sa_unary = unary_block(layer_ind, inputs, sa, radius, fdim, config, training)
    with tf.variable_scope('ca_unary'):
        #ca = ca * dp_mask
        ca_unary = unary_block(layer_ind, inputs, ca, radius, fdim, config, training)
    with tf.variable_scope('no_attention_unary'):
        no_unary = unary_block(layer_ind, inputs, features, radius, fdim, config, training)
    with tf.variable_scope('psa_unary'):
        #psa = psa * dp_mask
        psa_unary = unary_block(layer_ind, inputs, psa, radius, fdim, config, training)

    return psa_unary, no_unary, sa_unary, ca_unary
...
#       \*******************/
#
def assemble_CNN_CAM_blocks(inputs, config, dropout_prob):
    """
    ...
    cams = []
    cla_logits = []
    no_att, dual_att, spa_att, cha_att = None, None, None, None
    ...
    for block_i, block in enumerate(config.architecture):
        ...
        if 'attention' in block:
            with tf.variable_scope('layer_{:d}/{:s}_{:d}'.format(layer, block, block_in_layer)):
                ...
                no_att, dual_att, spa_att, cha_att = dual_attention_block(layer,
                                                                          inputs,
                                                                          features,
                                                                          r,
                                                                          fdim,
                                                                          ...
                                                                          training,
                                                                          dropout_prob)
            cams = [no_att, dual_att, spa_att, cha_att]

        elif 'global' in block:
            with tf.variable_scope('layer_{:d}/{:s}_{:d}_no_att'.format(layer, block, block_in_layer)):
                no_att_cla = global_average_block(layer,inputs,no_att,r,fdim,config,training)
            with tf.variable_scope('layer_{:d}/{:s}_{:d}_dual_att'.format(layer, block, block_in_layer)):
                dual_att_cla = global_average_block(layer, inputs, dual_att, r, fdim, config, training)
            with tf.variable_scope('layer_{:d}/{:s}_{:d}_spa_att'.format(layer, block, block_in_layer)):
                spa_att_cla = global_average_block(layer, inputs, spa_att, r, fdim, config, training)
            with tf.variable_scope('layer_{:d}/{:s}_{:d}_cha_att'.format(layer, block, block_in_layer)):
                cha_att_cla = global_average_block(layer, inputs, cha_att, r, fdim, config, training)
    ...
    return F, cams, cla_logits


def normalize_feature(features, inputs):
    c_features_ = tf.concat([features, tf.zeros((1, features.shape[1]), features.dtype)], axis=0)
    ...
    # Average features in each batch
    batch_features_sum = tf.reduce_sum(batch_features, axis=1)

    activations = features / tf.gather(batch_features_sum, inputs['last_batch_ind'])

    return activations


def assemble_FCNN_CAM_blocks(inputs, config, dropout_prob):
    """
    Definition of all the layers according to config
    ...
    F, cam, cla_logits= assemble_CNN_CAM_blocks(inputs, config, dropout_prob)
    #features = (cam[0]/(tf.reduce_max(cam[0], axis=1, keepdims=True)+1E-5)) + \
    #           (cam[1]/(tf.reduce_max(cam[1], axis=1, keepdims=True)+1E-5)) + \
    #           (cam[2]/(tf.reduce_max(cam[2], axis=1, keepdims=True)+1E-5)) + \
    #           (cam[3]/(tf.reduce_max(cam[3], axis=1, keepdims=True)+1E-5))
    #features = tf.math.maximum(normalize_feature(cam[0], inputs), normalize_feature(cam[1], inputs))
    #features = tf.math.maximum(features, normalize_feature(cam[2], inputs))
    #features = tf.math.maximum(features, normalize_feature(cam[3], inputs))
    #features = normalize_feature(cam[0], inputs) + normalize_feature(cam[1], inputs) + \
    #           normalize_feature(cam[2], inputs) + normalize_feature(cam[3], inputs)
    features = tf.math.maximum(cam[0], cam[1])
    features = tf.math.maximum(features, cam[2])
    features = tf.math.maximum(features, cam[3])
    #cam = F[-2]
    #logits = F[-1]
    #features = cam[0] + cam[1] +cam[2] +cam[3]
    #features = cam[3]
    ...
    return features, cla_logits, cam
...
def multi_classification_loss(logits, inputs, config):
    # Exclusive Labels cross entropy on each point

    cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(labels=inputs['cloud_labels'],
                                                                   logits=logits,
                                                                   name='xentropy')
    # Mean on batch

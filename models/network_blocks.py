...
def cam_block(layer_ind, inputs, features, radius, fdim, config, training, dropout_prob):

    #thing to add here

    features = unary_block(layer_ind, inputs, features, radius, config.num_classes, config, training)
    ...
    for block_i, block in enumerate(config.architecture):
        ...
        if 'cam' in block:
            ...
            with tf.variable_scope('cam'):
                features = cam_block(layer, inputs, features, r, fdim, config, training, dropout_prob)
            F += [features]
            cam = features
        else:
...
def assemble_FCNN_CAM_blocks(inputs, config, dropout_prob):
    ...
    F, cam = assemble_CNN_CAM_blocks(inputs, config, dropout_prob)
    features = cam
    #cam = F[-2]
    logits = F[-1]

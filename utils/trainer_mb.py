...
class ModelTrainer:
    ...
    def __init__(self, model, steps=0, epochs=0, restore_snap=None):

        self.training_step = steps
        self.training_epoch = epochs
        #current_lr = model.config.learning_rate * model.config.lr_decays[self.training_epoch]
        #print(current_lr)
        # Add training ops
    ...
    def add_train_ops(self, model):
        ...
        with tf.variable_scope('results'):
            ...
            if len(model.config.ignored_label_inds) > 0:
                ...
                new_logits = tf.gather(model.class_logits, inds, axis=0)
                new_labels = tf.gather(model.cloud_label, inds, axis=0)
                ...
                self.correct_prediction = tf.equal(tf.round(tf.nn.sigmoid(new_logits)), tf.round(tf.cast(new_labels,dtype=tf.float32)))
            ...
            '''
            # Metrics
            #self.correct_prediction = tf.nn.in_top_k(model.class_logits, model.labels, 1)
            self.correct_prediction0 = tf.equal(tf.round(tf.nn.sigmoid(model.class_logits[0])),
                                                tf.round(tf.cast(model.cloud_label,dtype=tf.float32)))
            self.accuracy0 = tf.reduce_mean(tf.cast(self.correct_prediction0, tf.float32))
            self.correct_prediction1 = tf.equal(tf.round(tf.nn.sigmoid(model.class_logits[1])),
                                                tf.round(tf.cast(model.cloud_label, dtype=tf.float32)))
            self.accuracy1 = tf.reduce_mean(tf.cast(self.correct_prediction1, tf.float32))
            self.correct_prediction2 = tf.equal(tf.round(tf.nn.sigmoid(model.class_logits[2])),
                                                tf.round(tf.cast(model.cloud_label, dtype=tf.float32)))
            self.accuracy2 = tf.reduce_mean(tf.cast(self.correct_prediction2, tf.float32))
            self.correct_prediction3 = tf.equal(tf.round(tf.nn.sigmoid(model.class_logits[3])),
                                                tf.round(tf.cast(model.cloud_label, dtype=tf.float32)))
    ...
    def train(self, model, dataset, debug_NaN=False):
        ...
        if self.training_epoch > 0:
            ...
            for i in range(1, self.training_epoch+1):
                current_lr *= model.config.lr_decays[i]
            op = self.learning_rate.assign(current_lr)
            self.sess.run(op)
        ...
        while self.training_epoch < model.config.max_epoch:
            ...
            try:
                ...
                if debug_NaN:
                    ...
                    _,  L_out0, L_out1, L_out2, L_out3, L_reg, L_p, probs, labels, \
                    class_logits, cam, inputs, acc0, acc1, acc2, acc3, lr \
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = 'Step {:08d} L_out0={:5.3f} L_out1={:5.3f} L_out2={:5.3f} L_out3={:5.3f}' \
                              ' L_reg={:5.3f} L_p={:5.3f}  Acc={:4.4f} Acc={:4.4f} Acc={:4.4f} Acc={:4.4f} lr={:4.4f}' \
                ...
                if model.config.saving:
                    ...
                    with open(join(model.saving_path, 'training.txt'), "a") as file:
                        message = '{:d} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.1f}\n'
    ...
    def cloud_validation_error(self, model, dataset):
        ...
        # Choose validation smoothing parameter (0 for no smothing, 0.99 for big smoothing)
        val_smooth = 0.95
        save_path = '/media/HD1/jiacheng/results/'
        ...
        if not hasattr(self, 'validation_logits'):
            self.validation_logits = [np.zeros((l.shape[0], nc_model)) for l in dataset.input_labels['validation']]
        ...
        for i0 in range(model.config.validation_size):
            try:
                ...
                stacked_probs, prob_logits, labels, batches, point_inds, cloud_inds = self.sess.run(ops, {model.dropout_prob: 1.0})
                t += [time.time()]
                ...
                for b_i, b in enumerate(batches):
                    ...
                    inds = point_inds[b]
                    c_i = cloud_inds[b_i]
                    logits = prob_logits[b]

                    # Update current probs in whole cloud
                    self.validation_probs[c_i][inds] = val_smooth * self.validation_probs[c_i][inds] \
                                                                + (1-val_smooth) * probs
                    self.validation_logits[c_i][inds] = val_smooth * self.validation_logits[c_i][inds] \
                                                       + (1 - val_smooth) * logits
        ...
        if model.config.saving and (self.training_epoch + 1) % model.config.snapshot_gap == 0:
            ...
            #val_prob_path = join(model.saving_path, 'val_probs_{:d}'.format(self.training_epoch))
            if not exists(val_path):
                makedirs(val_path)
            #if not exists(val_prob_path):
             #   makedirs(val_prob_path)
            ...
            for i, file_path in enumerate(files):
                if dataset.all_splits[i] == dataset.training_split:
                    ...
                    sub_logits = self.validation_logits[i_val]
                    ...
                    proj_logit = (sub_logits[dataset.validation_proj[i_val]]).astype(np.int32)
                    ...
                    prob_names = prob_names[1:]

                    # Save file
                    labels = dataset.validation_labels[i_val].astype(np.int32)
                    write_ply(val_name,
                              [points, rgb, preds, labels, proj_logit],
                              ['x', 'y', 'z', 'red', 'green', 'blue', 'preds', 'class']+prob_names)

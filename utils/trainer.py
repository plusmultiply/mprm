...
class ModelTrainer:
    ...
    def add_train_ops(self, model):
        ...
        with tf.variable_scope('results'):
            ...
            self.correct_prediction = tf.equal(tf.round(tf.nn.sigmoid(model.class_logits)), tf.round(tf.cast(model.cloud_label,dtype=tf.float32)))
            self.accuracy = tf.reduce_mean(tf.cast(self.correct_prediction, tf.float32))
        ...
        while self.training_epoch < model.config.max_epoch:
            ...
            try:
                ...
                if debug_NaN:
                    ...
                    _, L_out, L_reg, L_p, probs, labels, class_logits, inputs, acc = self.sess.run(ops, {model.dropout_prob: model.config.dropout_prob})

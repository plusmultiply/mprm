...
class Dataset:
    ...
    def init_input_pipeline(self, config):
        ...
        #print('Update network model in config')
        ...
        def re_init_input_pipeline(self, config):
            """
            Prepare the input pipeline with tf.Dataset class
            """
            ...
            print('Re-Initiating input pipelines')
    ...
    def tf_segmentation_cam_inputs(self,
                               config,
                               stacked_points,
                               stacked_features,
                               point_labels,
        ...
        for block_i, block in enumerate(config.architecture):
            ...
            if not ('pool' in block or 'strided' in block):
                layer_blocks += [block]
                if block_i < len(config.architecture) - 1 and not ('cam_block' in config.architecture[block_i + 1]):
                    continue
        ...
        # Batch unstacking (with last layer indices for optionnal classif loss)
        stacked_batch_inds_1 = self.tf_stack_batch_inds(input_batches_len[-1])
        last_batch_ind = self.tf_get_batch_inds(input_batches_len[-1])
    ...
    def tf_segmentation_attention_inputs(self,
                                   config,
                                   stacked_points,
                                   stacked_features,
                                   point_labels,
        ...
        for block_i, block in enumerate(config.architecture):
            ...
            if not ('pool' in block or 'strided' in block):
                layer_blocks += [block]
                if block_i < len(config.architecture) - 1 and not ('attention' in config.architecture[block_i + 1]):
                    continue
    ...
    def tf_segmentation_psp_inputs(self,
                                   config,
                                   stacked_points,
                                   stacked_features,
                                   point_labels,
        ...
        for block_i, block in enumerate(config.architecture):
            ...
            # Stop when meeting a global pooling or upsampling
            if 'global' in block or 'upsample' in block or 'cam' in block:
                break

            # Get all blocks of the layer
            if not ('pool' in block or 'strided' in block):
                layer_blocks += [block]
                if block_i < len(config.architecture) - 1 and not ('cam' in config.architecture[block_i + 1]):
                    continue
        ...
        ###############
        #Generate PSP neighbors
        ################
        #print('Generate PSP neighbors')
        input_psp_neighbors = []
        # New subsampling length
        dl = 2 * r_normal / (config.KP_extent * 2.5)
        # Subsampled points
        pool_p, pool_b = tf_batch_subsampling(input_points[-1], input_batches_len[-1], sampleDl=dl)
        # Radius of pooled neighbors
        r = r_normal * config.density_parameter / (config.KP_extent * 2.5)
        # Subsample indices
        pool_i = tf_batch_neighbors(pool_p, input_points[-1], pool_b, input_batches_len[-1], r)
        # Upsample indices (with the radius of the next layer to keep wanted density)
        up_i = tf_batch_neighbors(input_points[-1], pool_p, input_batches_len[-1], pool_b, 2 * r)
        pool_i = pool_i[:, :self.neighborhood_limits[-1]]
        up_i = up_i[:, :self.neighborhood_limits[-1]]
        #psp_batch_ind = self.tf_get_batch_inds(input_batches_len[-1])
        input_psp_neighbors = [pool_i, pool_b, up_i, pool_p]
        ...
        if object_labels is None:
            ...
            # list of network inputs
            li = input_points + input_neighbors + input_pools + input_upsamples + input_psp_neighbors
            li += [stacked_features, stacked_weights, stacked_batch_inds_0, stacked_batch_inds_1]
            li += [point_labels]
            li += [last_batch_ind]
            li += [input_batches_len[-1]]

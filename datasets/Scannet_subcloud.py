...
#      generate subcloud label and dataloader
#
# ----------------------------------------------------------------------------------------------------------------------
...
class ScannetDataset(Dataset):
    ...
    def load_subsampled_clouds(self, subsampling_parameter):
        ...
        print('number of training sample:', self.num_training)
        print('number of validation sample:', self.num_validation)
        ...
        for i, file_path in enumerate(files):
            ...
            if (not self.load_test) and 'train' in cloud_folder and cloud_name not in self.validation_clouds:
                proj_file = join(tree_path, '{:s}_proj_train.pkl'.format(cloud_name))
                if isfile(proj_file):
                    ...
                    tree_path = join(self.path, 'input_{:.3f}'.format(subsampling_parameter))
                    data = read_ply(join(tree_path, '{:s}.ply'.format(cloud_name)))
                    vertices = np.vstack((data['x'], data['y'], data['z'])).T
                    labels = data['class']
    ...
    def get_batch_gen(self, split, config):
        ...
        def spatially_regular_gen():
            ...
            for i in range(epoch_n):
                ...
                #cloud_ind = int(np.random.randint(len(self.anchors)))
                ...
                #print(n)
                if n == 0:
                    self.potentials[split][cloud_ind][point_ind] += 1
                ...
                if split != 'ERF':
                    ...
                    if split != 'training':
                        ...
                        self.potentials[split][cloud_ind][point_ind] += 0.01
                        self.min_potentials[split][cloud_ind] = float(np.min(self.potentials[split][cloud_ind]))
        ...
        gen_types = (tf.float32, tf.float32, tf.int32, tf.int32, tf.int32, tf.int32, tf.float32, tf.float32)
        gen_shapes = ([None, 3], [None, 6], [None], [None], [None], [None], [None, 20], [None, 20])
    ...
    def get_tf_mapping(self, config):
        ...
        def tf_map(stacked_points, stacked_colors, point_labels, stacks_lengths, point_inds, cloud_inds, cloud_labels, cloud_labels_all):
            """
            """
            ...
            input_list = self.tf_segmentation_attention_inputs(config,
                                                     stacked_points,
                                                     stacked_features,
                                                     point_labels,
                                                     stacks_lengths,
            ...
            input_list += [cloud_labels]
            input_list += [batch_inds]
            input_list += [cloud_labels_all]
            input_list += [stacks_lengths]
    ...
    def load_evaluation_points_on_train(self, cloud_name):
        """
        Load points (from test or validation split) on which the metrics should be evaluated
        """
        subsampling_parameter = 0.04
